...
const SearchableSelect = ({
  ...
  const filteredOptions = options.filter((option) =>
    option.CollegeName.toLowerCase().includes(searchTerm.toLowerCase())
  );
          ...
          <>
            ...
            >
              <div className="flex items-center">
                <input
                  type="text"
                  className="w-full rounded px-3 text-gray-800 py-2 bg-gray-50 outline-none"
            ...
            {isOpen && (
              <div className="absolute z-50 mt-1 w-full max-h-60 flex flex-col gap-3 overflow-auto rounded-md bg-white border border-gray-200 shadow-lg">
                {/* Other Option always visible at top */}
                <div
                ...
                {filteredOptions.length > 0 ? (
                  filteredOptions.map((option) => (
                    <div
                      key={option.CollegeID}
                      className={`cursor-pointer px-3 py-2 text-gray-800 hover:bg-gray-100 ${
                        field.value === option.CollegeName
                          ? "bg-indigo-50 text-indigo-600"
                          : ""
                      }`}
                      onClick={() => handleSelect(option.CollegeName)}
                    >
                      {option.CollegeName}
                    </div>
                  ))
